...
import { upsertTpCustomer, type CustomerFormState } from '../actions'
import type { TpCustomer, TradePartner } from '@/lib/types'

interface Props {
  customer?: TpCustomer
  tradePartners: Pick<TradePartner, 'id' | 'name'>[]
}

const INITIAL_STATE: CustomerFormState = null

export function TpCustomerForm({ customer, tradePartners }: Props) {
  const [state, action, pending] = useActionState(upsertTpCustomer, INITIAL_STATE)
  ...
  return (
    <form action={action} className="space-y-6 max-w-2xl">
      ...
      <div>
        <label htmlFor="trade_partner_id" className="block text-sm font-medium text-gray-700">
          Trade partner <span className="text-red-500">*</span>
        </label>
        <select
          id="trade_partner_id"
          name="trade_partner_id"
          required
          defaultValue={customer?.trade_partner_id ?? ''}
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        >
          {pending ? 'Saving…' : isEdit ? 'Save changes' : 'Add TP customer'}
        </button>
        <Link
