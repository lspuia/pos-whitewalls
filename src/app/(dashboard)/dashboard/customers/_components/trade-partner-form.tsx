...
import { upsertTradePartner, type CustomerFormState } from '../actions'
...
export function TradePartnerForm({ partner }: Props) {
  const [state, action, pending] = useActionState(upsertTradePartner, INITIAL_STATE)
  ...
  return (
    <form action={action} className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        >
          {pending ? 'Saving…' : isEdit ? 'Save changes' : 'Add trade partner'}
        </button>
        <Link
